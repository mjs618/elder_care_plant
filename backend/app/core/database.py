...
# ── Engine ───────────────────────────────────────────────────────────────────
# NullPool is safe for environments using pgBouncer / serverless functions.
# Switch to AsyncAdaptedQueuePool for persistent long-running server processes.
engine = create_async_engine(
    settings.DATABASE_URL,
    echo=settings.DEBUG,
    pool_size=settings.DATABASE_POOL_SIZE,
    max_overflow=settings.DATABASE_MAX_OVERFLOW,
    # NullPool is recommended for scripts; leave default pool for the server
    # poolclass=NullPool,
)
